...
void CMFCApplication4Dlg::OnBnClickedOk()
{
	...
	GetDlgItem(IDC_IDNAME)->GetWindowTextA(m_userName);
	GetDlgItem(IDC_PASSWORD)->GetWindowTextA(m_userPassword);
	if (strlen(m_userName) < 1) {
		MessageBox(TEXT("请输入用户名！"),TEXT("提示"));
		return;
	}
	if (strlen(m_userPassword) < 1) {
		MessageBox(TEXT("请输入密码！"), TEXT("提示"));
		return;
	}
	if (strcmp(m_userName, "admin") == 0 && strcmp(m_userPassword, "admin123") == 0) {
		MessageBox(TEXT("登录成功！"),TEXT("提示"));
		CDialogEx::OnOK();
		CSystemDlg dlg;
		dlg.DoModal();
	}
	else if(strcmp(m_userName, "张三") == 0 && strcmp(m_userPassword, "zhangsan123") == 0) {
		MessageBox(TEXT("登录成功！"), TEXT("提示"));
		CDialogEx::OnOK();
		CSystemDlg dlg;
		dlg.DoModal();
